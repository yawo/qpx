              ...
              start_airport_code: start_airport_code,
                end_airport_code: end_airport_code,
                     ...
                     coordinates: self.city_top_airport(end_airport_data['city']).values_at('longitude','latitude'),
                           title: '', # Evol
                        prefered: false,
                      start_time: nil, #Time.parse(firstLeg['departureTime']).strftime('%H.%M').to_f, #conserve current grapy system
                        end_time: nil, #Time.parse(lastLeg['arrivalTime']).strftime('%H.%M').to_f,
